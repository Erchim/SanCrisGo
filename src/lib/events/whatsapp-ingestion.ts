...
interface CandidateRow { id: string; moderation_sent_at: string | null }
...
export class WhatsAppEventIngester {
  ...
  async ingest(input: WhatsAppEventInput): Promise<IngestionResult> {
    ...
    if (existing) return { candidateId: existing.id, moderationSent: Boolean(existing.moderation_sent_at) };
    ...
    if (inserted.error) {
      ...
      if (duplicate) return { candidateId: duplicate.id, moderationSent: Boolean(duplicate.moderation_sent_at) };
